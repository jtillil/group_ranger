...
void TreeGroup::grow(std::vector<double>* variable_importance) {
...
void TreeGroup::predict(const Data* prediction_data, bool oob_prediction) {
...
void TreeGroup::computePermutationImportance(std::vector<double>& forest_importance, std::vector<double>& forest_variance,
...
// #nocov start
void TreeGroup::appendToFile(std::ofstream& file) {
...
size_t TreeGroup::dropDownSamplePermuted(size_t permuted_varID, size_t sampleID, size_t permuted_sampleID) {
...
void TreeGroup::permuteAndPredictOobSamples(size_t permuted_varID, std::vector<size_t>& permutations) {
...
void TreeGroup::bootstrapWithoutReplacementClassWise() {
  // Empty on purpose (virtual function only implemented in classification and probability)
}

void TreeGroup::setManualInbag() {
