...
void ForestClassificationGroup::initInternal() {
...
void ForestClassificationGroup::growInternal() {
  trees.reserve(num_trees);
  for (size_t i = 0; i < num_trees; ++i) {
    trees.push_back(
...
void ForestClassificationGroup::allocatePredictMemory() {
...
void ForestClassificationGroup::predictInternal(size_t sample_idx) {
  if (predict_all || prediction_type == TERMINALNODES) {
...
void ForestClassificationGroup::computePredictionErrorInternal() {
...
// #nocov start
void ForestClassificationGroup::writeOutputInternal() {
  if (verbose_out) {
    *verbose_out << "Tree type:                         " << "Classification" << std::endl;
  }
}

void ForestClassificationGroup::writeConfusionFile() {
...
void ForestClassificationGroup::writePredictionFile() {
...
void ForestClassificationGroup::saveToFileInternal(std::ofstream& outfile) {
...
void ForestClassificationGroup::loadFromFileInternal(std::ifstream& infile) {
...
double ForestClassificationGroup::getTreePrediction(size_t tree_idx, size_t sample_idx) const {
  const auto& tree = dynamic_cast<const TreeClassificationGroup&>(*trees[tree_idx]);
  return tree.getPrediction(sample_idx);
}

size_t ForestClassificationGroup::getTreePredictionTerminalNodeID(size_t tree_idx, size_t sample_idx) const {
  const auto& tree = dynamic_cast<const TreeClassificationGroup&>(*trees[tree_idx]);
