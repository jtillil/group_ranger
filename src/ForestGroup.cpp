...
void ForestGroup::init(std::unique_ptr<Data> input_data, uint mtry, std::string output_prefix,
...
void ForestGroup::run(bool verbose, bool compute_oob_error) {
...
void ForestGroup::writeImportanceFile() {
...
void ForestGroup::grow() {
  ...
  for (uint i = 0; i < num_threads; ++i) {
    ...
    threads.emplace_back(&ForestGroup::growTreesInThread, this, i, &(variable_importance_threads[i]));
  }
...
void ForestGroup::predict() {
  ...
  threads.reserve(num_threads);
  for (uint i = 0; i < num_threads; ++i) {
    threads.emplace_back(&ForestGroup::predictTreesInThread, this, i, data.get(), false);
  }
  ...
  progress = 0;
  for (uint i = 0; i < num_threads; ++i) {
    threads.emplace_back(&ForestGroup::predictInternalInThread, this, i);
  }
...
void ForestGroup::computePredictionError() {
  ...
  progress = 0;
  for (uint i = 0; i < num_threads; ++i) {
    threads.emplace_back(&ForestGroup::predictTreesInThread, this, i, data.get(), true);
  }
...
void ForestGroup::computePermutationImportance() {
  ...
  for (uint i = 0; i < num_threads; ++i) {
    ...
    threads.emplace_back(&ForestGroup::computeTreePermutationImportanceInThread, this, i,
...
void ForestGroup::growTreesInThread(uint thread_idx, std::vector<double>* variable_importance) {
...
void ForestGroup::predictTreesInThread(uint thread_idx, const Data* prediction_data, bool oob_prediction) {
  if (thread_ranges.size() > thread_idx + 1) {
    for (size_t i = thread_ranges[thread_idx]; i < thread_ranges[thread_idx + 1]; ++i) {
...
void ForestGroup::predictInternalInThread(uint thread_idx) {
  // Create thread ranges
...
void ForestGroup::computeTreePermutationImportanceInThread(uint thread_idx, std::vector<double>& importance,
...
// #nocov start
void ForestGroup::loadFromFile(std::string filename) {
  if (verbose_out)
...
void ForestGroup::loadDependentVariableNamesFromFile(std::string filename) {
...
void ForestGroup::setSplitWeightVector(std::vector<std::vector<double>>& split_select_weights) {
...
void ForestGroup::setAlwaysSplitVariables(const std::vector<std::string>& always_split_variable_names) {
...
void ForestGroup::showProgress(std::string operation, size_t max_progress) {
